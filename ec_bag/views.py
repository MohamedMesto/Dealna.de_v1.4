    ...
    if size:
        if item_id in list(ec_bag.keys()):
            if size in ec_bag[item_id]['items_by_size'].keys():
                ...
                messages.success(request, f'Updated size {size.upper()} {ec_product.name} quantity to {ec_bag[item_id]["items_by_size"][size]}')
            else:
                ec_bag[item_id]['items_by_size'][size] = quantity
                messages.success(request, f'Added size {size.upper()} {ec_product.name} to your bag')
        else:
            ec_bag[item_id] = {'items_by_size': {size: quantity}}
            messages.success(request, f'Added size {size.upper()} {ec_product.name} to your bag')
    else:
        if item_id in list(ec_bag.keys()):
            ...
            messages.success(request, f'Updated {ec_product.name} quantity to {ec_bag[item_id]}')
        else:
            ec_bag[item_id] = quantity
    ...
    if size:
        if quantity > 0:
            ...
            if not ec_bag[item_id]['items_by_size']:
                ec_bag.pop(item_id)
            messages.success(request, f'Removed size {size.upper()} {ec_product.name} from your bag')
    else:
        if quantity > 0:
            ec_bag[item_id] = quantity
            messages.success(request, f'Updated {ec_product.name} quantity to {ec_bag[item_id]}')
            ...
            ec_bag.pop(item_id)
            messages.success(request, f'Removed {ec_product.name} from your bag')
